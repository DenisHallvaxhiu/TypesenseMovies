...
module.exports = (async () => {
  ...
  const schema = {
    ...
    try{
        ...
        console.log("Found existing collections of movies")
        console.log(JSON.stringify(collection,null,2))

        if(collection.num_documents !== movies.length){
            console.log("Collections had different number of documents than data")
    ...
    movies.forEach(async (movie) =>{
        ...
        movie.genres.forEach((genre,index)=>{
            movie[`genres.lvl${index}`] =[
                movie.genres.slice(0,index + 1).join(">")
            ]
        })
    })

    try {
        const returnData = await typesense.collections("movies").documents().import(movies)
